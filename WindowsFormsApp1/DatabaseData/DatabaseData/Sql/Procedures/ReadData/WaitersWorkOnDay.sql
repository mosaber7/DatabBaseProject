...
AS
	...
	SELECT W.FirstName, W.LastName,
		SUM(DATEDIFF(minute, S.ClockInTime, S.ClockOutTime) / 60) AS HoursWorked,
		SUM(DATEDIFF(minute, S.ClockInTime, S.ClockOutTime) * W.Salary / 60) AS WorkersEarnings,
	...
	WHERE YEAR(S.ClockInTime) = @YearOfDate AND
		...
		O.[Status] = 2
	GROUP BY W.FirstName, W.LastName, W.Salary